<template>
    <div>
        <el-row>
            <h1>{{idQuestion+' '+questionInfo.title}}</h1>
        </el-row>
        <el-collapse v-model="activeNames">
            <el-collapse-item title="Question Description" name="1">
                <el-row>
...
<!--            <p>正确答案: <span>{{result['right_answer']}}</span></p>-->
            <p>Error: <span style="color:red ;">{{result['syntax_error_msg']}}</span></p>
<!--            <p>结果对比:</p>-->
            <el-table :data="result['compare_result']">
                <el-table-column label="Your Result" prop="your_result"></el-table-column>
                <el-table-column label="Correct Result" prop="right_result"></el-table-column>
            </el-table>
<!--            <p>子句:</p>-->
        ...
        methods:{
            handleSubmit(){
                ...
                this.$axios.post('/question/'+this.idQuestion+'/submit',{
                  ...
                  // if (this.result['syntax_error_msh']==='')
